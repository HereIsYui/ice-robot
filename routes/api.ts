...
apiRouter.get("/bribe", jwt, async (req, res) => {
  ...
  if (num <= 0) {
    await chat.chatRoomSend(`@${user.user} 小气鬼${nick_name},一个${item}都不给我,小冰亲密度${intimacy}`);
  } else if (num <= 5) {
    await chat.chatRoomSend(`@${user.user} 谢谢小气鬼${nick_name}送的${num}个${item},小冰亲密度+${intimacy}`);
  } else {
    await chat.chatRoomSend(`@${user.user} 谢谢${nick_name}送的${num}个${item}:heartbeat:小冰亲密度+${intimacy}`);
  }
  res.send({ code: 0, msg: "success" });
});

apiRouter.get("/XiaoIceSendMsg", jwt, async (req, res) => {
    let msg = decodeURI(req.query.msg?.toString() ?? "");
    chat.chatRoomSend(msg);
});
